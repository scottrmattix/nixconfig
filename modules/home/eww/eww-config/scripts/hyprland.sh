...
num_workspaces() {
  hyprctl workspaces -j | jq 'map(.id) | .[]'
}
...
workspaces (){
  ...
  hyprctl workspaces -j | jq 'map(.id) | .[]' | jq --argjson windows "${WORKSPACE_WINDOWS}" --slurp -Mc 'map(tostring) | map({id: ., windows: ($windows[.]//0)})'
}
...
if [[ $1 == 'workspaces' ]]; then
    ...
    socat -u UNIX-CONNECT:/tmp/hypr/"$HYPRLAND_INSTANCE_SIGNATURE"/.socket2.sock - | while read -r line; do
        ...
        \"workspaces\": $(workspaces | jq '.|sort_by(.id)'),
            \"active\": $active,
