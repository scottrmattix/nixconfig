...
rec {
  ...
  */
  ...
  /*
  Similar to import modules, but instead of taking a module directly it takes functions that output NixosModules

  @param directory location of Nix Module Functions
  @param args argument to pass to each Nix Module Functions
  @return     attrset of Nix Modules
  */
  importModulesWith = directory: args:
    with lib; let
      moduleFun = importModules directory;
    in
    mapAttrs (n : v: ( (import v) args)) moduleFun;
}
